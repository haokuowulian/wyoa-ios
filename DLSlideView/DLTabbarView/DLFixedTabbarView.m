...
- (void)commonInit{
    ...
    bottomView_=[[UIView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-kTrackViewHeight, self.bounds.size.width, 0.5)];
    [bottomView_ setBackgroundColor:[UIColor colorWithHexString:@"999999"]];
...
//            break;
//    }
    float trackX = self.bounds.size.width/self.tabbarItems.count*self.selectedIndex+150/self.tabbarCount*0.5;
    trackView_.frame = CGRectMake(trackX, self.bounds.size.height-kTrackViewHeight-1, width-150/self.tabbarCount, kTrackViewHeight);
