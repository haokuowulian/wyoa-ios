    ...
    return ^(JXTAlertActionBlock actionBlock) {
        if (self.jxt_alertActionArray.count > 0)
        {
            ...
            [self.jxt_alertActionArray enumerateObjectsUsingBlock:^(JXTAlertActionModel *actionModel, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                //可利用这个改变字体颜色，但是不推荐！！！
                if([actionModel.title isEqualToString:@"取消"]){
                     [alertAction setValue:[UIColor colorWithRed:255/255.0 green:80/255.0 blue:70/255.0 alpha:1.0f] forKey:@"titleTextColor"];
                }else{
                     [alertAction setValue:[UIColor colorWithRed:3/255.0 green:123/255.0 blue:255/255.0 alpha:1.0f] forKey:@"titleTextColor"];
                }
