...
@interface BaoXiuViewController ()
...
- (IBAction)timeClick:(id)sender {
     [self selectDateTime:self.timeText];
}
...
-(void)getShenPiRen{
    ...
    NSString *url=[NSString stringWithFormat:@"%@%@?apikey=%@",baseUrl,@"oaOffice/getInfix.do",apikey];
...
-(BOOL)checkIsEmpty{
    if(self.wupinText.text.length>0&&
       self.placeText.text.length>0&&
       self.timeText.text.length>0&&
...
-(void)submit{
    ...
    NSString *userId=[userDefault objectForKey:@"userId"];
    NSString *apikey=[userDefault objectForKey:@"apikey"];
    NSString *url=[NSString stringWithFormat:@"%@%@?apikey=%@",baseUrl,@"oaOffice/launchFix.do",apikey];
    
    NSDictionary *temp=@{@"userId":userId,
                         @"fillformDate":[TimeUtil getCurrentTimesNOHour],
                         @"expectfixDate":self.timeText.text,
                         @"fixItems":self.wupinText.text,
                         @"damage":self.detailText.text,
                         @"fixAddress":self.placeText.text,
                         @"notes":self.detailText.text,
