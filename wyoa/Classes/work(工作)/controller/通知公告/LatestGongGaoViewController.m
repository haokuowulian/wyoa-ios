...
@interface LatestGongGaoViewController ()
@property(nonatomic,assign)NSInteger pageIndex;
@property(nonatomic,strong)NSMutableArray *newsArray;
@end

@implementation LatestGongGaoViewController
...
- (void)viewDidLoad {
    ...
    [MBProgressHUD showMessage:@"正在获取最新公告..."];
    self.pageIndex=0;
    [self getNews];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UIStoryboard *storyboard=[UIStoryboard storyboardWithName:@"LatestGongGaoDetail" bundle:nil];
    LatestGongGaoDetailViewController *controller=[storyboard instantiateViewControllerWithIdentifier:@"latestGongGaoDetail"];
    NewsDetailBean *bean=self.newsArray[indexPath.row];
    controller.title=@"公告详情";
...
-(void)getNews{
    ...
    NSString *url=[NSString stringWithFormat:@"%@%@?apikey=%@",baseUrl,@"noteNew/getInNewsSortList.do",apikey];
