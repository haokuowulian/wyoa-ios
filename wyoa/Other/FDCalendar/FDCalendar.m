...
- (UIView *)datePickerView {
    if (!_datePickerView) {
        ...
        [okButton setTitleColor:kThemeColor forState:UIControlStateNormal];
...
- (void)setupWeekHeader {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (i == 0 || i == count - 1) {
            weekdayLabel.textColor = kThemeColor;
        } else {
...
- (void)calendarItem:(FDCalendarItem *)item didSelectedDate:(NSDate *)date {
    ...
    [self setCurrentDate:self.date];
    //创建一个消息对象
    NSNotification * notice = [NSNotification notificationWithName:@"sendDate" object: self.date];
    //发送消息
