...
@property(nonatomic,assign)NSInteger startIndexFirstDay;
@end
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        self.doneBtn = ({
            ...
            [btn setBackgroundColor:[UIColor hexColorWithString:@"23A4FE"]];
...
- (void)drawTitleView {
    ...
    for(int i = 0 ;i < 7; i++)
    {
        ...
        lab.textColor       = [UIColor hexColorWithString:@"2b2b2b"];
        lab.textAlignment   = NSTextAlignmentCenter;
...
//        monthLab.textColor       = [UIColor hexColorWithString:@"2b2b2b"];
...
- (void)showDateView {
    ...
    CGFloat startDayIndex       = [self calculateStartIndex:self.firstDay];
    self.startIndexFirstDay=startDayIndex;
    ...
    for(int i = startDayIndex; i < [self getNumberOfDaysInMonth]+startDayIndex;i++) {
        //需要换行且不在第一行
        if (i % kCol == 0 && i != 0) {
...
-(void)tap:(UITapGestureRecognizer *)gesture{
    ...
    if (CGRectContainsPoint(_touchRect, point)) {
        ...
        if(index<=[self getNumberOfDaysInMonth]+self.startIndexFirstDay-1){
            [self clickForIndex:index];
        }
    }
}
-(void)tapDismiss:(UITapGestureRecognizer *)gesture{
...
- (void)doneBtnClick:(id)sender {
    if (_complete) {
        _complete([_selectArray mutableCopy],self.textField);
    }
    [self hide];
}
...
{
    NSString *str_date=[NSString stringWithFormat:@"%d-%d",self.year,self.month];
    NSDate *currentDate=[TimeUtil stringToDate:str_date];
